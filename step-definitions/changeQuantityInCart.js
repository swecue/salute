...
module.exports = function() {
	...
	this.Then(/^the quantity of the product should be five$/, function (callback) {
         ...
         assert(cart.thingsToBuy[index].quantity === 5, "The quantity was not updated correctly");
	...
	this.Then(/^the quantity of the product should have changed to six$/, function (callback) {
         index = cart.findProductInCart(addedProduct)
         assert(index !== -1, "The product is no longer in the shopping cart");

         assert(cart.thingsToBuy[index].quantity === 6, "The quantity was not updated correctly");
  ...
  this.Given(/^that the cart has one product$/, function (callback) {
	...
	this.When(/^you try to change quantity of another product that is not in the cart$/, function (callback) {
  ...
  this.Given(/^that the cart has three diffrent products each with a quantity of one$/, function (callback) {
         cart = new ShoppingCart();
         addedProduct = app.products[0];
  ...
  this.When(/^you change quantity of one of the products to ten$/, function (callback) {
         newQuantity = 10;
         cart.changeQuantity(addedProduct, newQuantity);
         callback();
       });

   this.Then(/^the changed product should have a quantity of ten$/, function (callback) {
         index = cart.findProductInCart(addedProduct)
         assert(index !== -1, "The product is no longer in the shopping cart");

         assert(cart.thingsToBuy[index].quantity === 10, "The quantity was not updated correctly");
         callback();
       });

   this.Then(/^the other two products should still have a quantity of one each$/, function (callback) {
         index = cart.findProductInCart(secondProduct)
         assert(index !== -1, "The product is no longer in the shopping cart");
         assert(cart.thingsToBuy[index].quantity === 1, "The quantity was changed");
