...
module.exports = function() {
    ...
    this.Given(/^you have a product with a quantity of "([^"]*)" in the shopping cart$/, function(arg1, callback) {
    ...
    this.When(/^you remove the product from the shopping cart$/, function(callback) {
        // Use the remove method to remove the product from cart     
        cart.remove(aProduct);
        callback();
    });

    this.Then(/^the shopping cart will not contain the removed product$/, function(callback) {
        ...
        // shopping cart"
        assert(cart.findProductInCart(aProduct) === -1, "The product was not removed from the shopping cart");
        callback();
    });


    this.Given(/^you have a number of different products in the shopping cart$/, function(callback) {
        // Create a shopping cart
        cart = new ShoppingCart();
    ...
    this.When(/^you remove one of the products from the shopping cart$/, function(callback) {
    ...
    this.Then(/^the shopping cart will only contain the products that was not removed$/, function(callback) {
        ...
        // shopping cart"
        assert(cart.findProductInCart(secondProduct) === -1, "The product was not removed from the shopping cart");
        ...
        assert(cart.findProductInCart(firstProduct) !== -1, "The product is not in the shopping cart");

        assert(cart.findProductInCart(thirdProduct) !== -1, "The product is not in the shopping cart");
    ...
    this.When(/^you try to remove another product that is not in the shopping cart$/, function(callback) {
